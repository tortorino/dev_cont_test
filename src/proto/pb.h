...
#define PB_STATIC_ASSERT_MSG_(MSG, LINE, COUNTER) \
  pb_static_assertion_##MSG##_##LINE##_##COUNTER
...
#define PB_SIZE_MAX ((pb_size_t)-1)
  ...
  struct pb_msgdesc_s
  {
    ...
    bool (*field_callback)(pb_istream_t *istream,
 ...
 */
  ...
  PB_STATIC_ASSERT(sizeof(uint64_t) == 2 * sizeof(uint32_t),
                   UINT64_T_WRONG_SIZE)
#endif
...
#define PB_BYTES_ARRAY_T_ALLOCSIZE(n) \
  ...
  struct pb_callback_s
  {
    ...
    union
    {
      ...
      bool (*encode)(pb_ostream_t *stream,
  ...
  extern bool pb_default_field_callback(pb_istream_t *istream,
                                        pb_ostream_t *ostream,
                                        const pb_field_t *field);
...
#define PB_BIND(msgname, structname, width)                             \
  ...
  const pb_msgdesc_t *const structname##_submsg_info[]                  \
    = { msgname##_FIELDLIST(PB_GEN_SUBMSG_INFO, structname) NULL };     \
...
#define PB_GEN_REQ_FIELD_COUNT(structname, atype, htype, ltype, fieldname, \
                               tag)                                        \
...
#define PB_GEN_FIELD_INFO_AUTO(structname, atype, htype, ltype, fieldname, \
                               tag)                                        \
  PB_FIELDINFO_AUTO2(                                                      \
    PB_FIELDINFO_WIDTH_AUTO(_PB_ATYPE_##atype, _PB_HTYPE_##htype,          \
                            _PB_LTYPE_##ltype),                            \
    tag, PB_ATYPE_##atype | PB_HTYPE_##htype | PB_LTYPE_MAP_##ltype,       \
    PB_DATA_OFFSET_##atype(_PB_HTYPE_##htype, structname, fieldname),      \
    PB_DATA_SIZE_##atype(_PB_HTYPE_##htype, structname, fieldname),        \
...
#define PB_FIELDINFO_AUTO2(width, tag, type, data_offset, data_size,        \
                           size_offset, array_size)                         \
  PB_FIELDINFO_AUTO3(width, tag, type, data_offset, data_size, size_offset, \
                     array_size)

#define PB_FIELDINFO_AUTO3(width, tag, type, data_offset, data_size,   \
                           size_offset, array_size)                    \
  PB_FIELDINFO_##width(tag, type, data_offset, data_size, size_offset, \
                       array_size)

/* X-macro for generating asserts that entries fit in struct_field_info[] array.
 * The structure of macros here must match the structure above in
 * PB_GEN_FIELD_INFO_x(), but it is not easily reused because of how macro
 * substitutions work. */
#define PB_GEN_FIELD_INFO_ASSERT_1(structname, atype, htype, ltype, fieldname, \
                                   tag)                                        \
...
#define PB_GEN_FIELD_INFO_ASSERT_2(structname, atype, htype, ltype, fieldname, \
                                   tag)                                        \
...
#define PB_GEN_FIELD_INFO_ASSERT_4(structname, atype, htype, ltype, fieldname, \
                                   tag)                                        \
...
#define PB_GEN_FIELD_INFO_ASSERT_8(structname, atype, htype, ltype, fieldname, \
                                   tag)                                        \
...
#define PB_GEN_FIELD_INFO_ASSERT_AUTO(structname, atype, htype, ltype, \
                                      fieldname, tag)                  \
...
#define PB_FIELDINFO_ASSERT_AUTO2(width, tag, type, data_offset, data_size, \
                                  size_offset, array_size)                  \
  PB_FIELDINFO_ASSERT_AUTO3(width, tag, type, data_offset, data_size,       \
                            size_offset, array_size)

#define PB_FIELDINFO_ASSERT_AUTO3(width, tag, type, data_offset, data_size,   \
                                  size_offset, array_size)                    \
  PB_FIELDINFO_ASSERT_##width(tag, type, data_offset, data_size, size_offset, \
                              array_size)
...
#define PB_SO_PB_HTYPE_ONEOF(structname, fieldname)                 \
  PB_SO_PB_HTYPE_ONEOF2(structname, PB_ONEOF_NAME(FULL, fieldname), \
                        PB_ONEOF_NAME(UNION, fieldname))
...
#define PB_SUBMSG_INFO_ONEOF(ltype, structname, fieldname)                  \
  PB_SUBMSG_INFO_ONEOF2(ltype, structname, PB_ONEOF_NAME(UNION, fieldname), \
                        PB_ONEOF_NAME(MEMBER, fieldname))
...
#define PB_SUBMSG_DESCRIPTOR(t) &(t##_msg),
   ...
   * of the field type.
   *
   * Descriptor size is encoded as 0 = 1 word, 1 = 2 words, 2 = 4 words, 3 = 8
   * words.
   *
   * Formats, listed starting with the least significant bit of the first word.
   * 1 word:  [2-bit len] [6-bit tag] [8-bit type] [8-bit data_offset] [4-bit
   * size_offset] [4-bit data_size]
   *
   * 2 words: [2-bit len] [6-bit tag] [8-bit type] [12-bit array_size] [4-bit
   * size_offset] [16-bit data_offset] [12-bit data_size] [4-bit tag>>6]
   *
...
#define PB_FIELDINFO_8(tag, type, data_offset, data_size, size_offset, \
                       array_size)                                     \
  (3 | (((tag) << 2) & 0xFF) | ((type) << 8)),                         \
    ((uint32_t)(int_least8_t)(size_offset)                             \
     | (((uint32_t)(tag) << 2) & 0xFFFFFF00)),                         \
...
/* These assertions verify that the field information fits in the allocated
 * space. The generator tries to automatically determine the correct width that
 * can fit all data associated with a message. These asserts will fail only if
 * there has been a problem in the automatic logic - this may be worth reporting
 * as a bug. As a workaround, you can increase the descriptor width by defining
 * PB_FIELDINFO_WIDTH or by setting descriptorsize option in .options file.
 */
#define PB_FITS(value, bits) ((uint32_t)(value) < ((uint32_t)1 << bits))
#define PB_FIELDINFO_ASSERT_1(tag, type, data_offset, data_size, size_offset, \
                              array_size)                                     \
  PB_STATIC_ASSERT(PB_FITS(tag, 6) && PB_FITS(data_offset, 8)                 \
                     && PB_FITS(size_offset, 4) && PB_FITS(data_size, 4)      \
                     && PB_FITS(array_size, 1),                               \
                   FIELDINFO_DOES_NOT_FIT_width1_field##tag)

#define PB_FIELDINFO_ASSERT_2(tag, type, data_offset, data_size, size_offset, \
                              array_size)                                     \
  PB_STATIC_ASSERT(PB_FITS(tag, 10) && PB_FITS(data_offset, 16)               \
                     && PB_FITS(size_offset, 4) && PB_FITS(data_size, 12)     \
                     && PB_FITS(array_size, 12),                              \
                   FIELDINFO_DOES_NOT_FIT_width2_field##tag)

#ifndef PB_FIELD_32BIT
/* Maximum field sizes are still 16-bit if pb_size_t is 16-bit */
#define PB_FIELDINFO_ASSERT_4(tag, type, data_offset, data_size, size_offset, \
                              array_size)                                     \
  PB_STATIC_ASSERT(PB_FITS(tag, 16) && PB_FITS(data_offset, 16)               \
                     && PB_FITS((int_least8_t)size_offset, 8)                 \
                     && PB_FITS(data_size, 16) && PB_FITS(array_size, 16),    \
                   FIELDINFO_DOES_NOT_FIT_width4_field##tag)

#define PB_FIELDINFO_ASSERT_8(tag, type, data_offset, data_size, size_offset, \
                              array_size)                                     \
  PB_STATIC_ASSERT(PB_FITS(tag, 16) && PB_FITS(data_offset, 16)               \
                     && PB_FITS((int_least8_t)size_offset, 8)                 \
                     && PB_FITS(data_size, 16) && PB_FITS(array_size, 16),    \
 ...
 * Note that the checks are against 31 bits to avoid compiler warnings about
 * shift wider than type in the test. I expect that there is no reasonable use
 * for >2GB messages with nanopb anyway.
 */
#define PB_FIELDINFO_ASSERT_4(tag, type, data_offset, data_size, size_offset, \
                              array_size)                                     \
  PB_STATIC_ASSERT(PB_FITS(tag, 30) && PB_FITS(data_offset, 31)               \
                     && PB_FITS(size_offset, 8) && PB_FITS(data_size, 31)     \
